    ...
    public static void delete(@RequestBody String mail) throws IOException {
        ...
        while (j!=s.length) {
            ...
            if(values[1].equals("sent")) {
                ...
                path2 = "database\\"+l.get(j).getString("from")+"\\trash\\trash.json";
            } else if(values[1].equals("inbox")){
                path = "database\\"+l.get(j).getString("to")+"\\inbox\\inbox.json";
                path2 = "database\\"+l.get(j).getString("to")+"\\trash\\trash.json";
            }else{
                path = "database\\"+l.get(j).getString("to")+"\\starred\\starred.json";
                path2 = "database\\"+l.get(j).getString("to")+"\\trash\\trash.json";
            }
            ...
            if(values[1].equals("sent")) {
                fw = new FileWriter("database\\"+l.get(j).getString("from")+"\\sent\\sent.json");
                fw2 = new FileWriter("database\\"+l.get(j).getString("from")+"\\trash\\trash.json");
            } else if(values[1].equals("inbox")){
                fw = new FileWriter("database\\"+l.get(j).getString("to")+"\\inbox\\inbox.json");
                fw2 = new FileWriter("database\\"+l.get(j).getString("to")+"\\trash\\trash.json");
            }
            else {
